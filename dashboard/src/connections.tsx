...
function ProviderOverview() {
  ...
  return <section>
    ...
    <div className="provider-grid">{!loading && !error && visibleProviders.map((provider) => {
      const rows = connections.filter((connection) => connection.provider === provider);
      ...
      const meta = PROVIDERS[provider];
      return <a className="card provider-card provider-link" href={`#/providers/${provider}`} key={provider}>
        <div className="provider-card-head"><ProviderIcon provider={provider} alt={`${meta.label} logo`} size={42} /><div><h3>{meta.label}</h3><p>{meta.description}</p></div><Icon>open_in_new</Icon><div className="provider-status">{ready > 0 && <Badge variant="success" dot>{ready} Ready</Badge>}{errors > 0 && <Badge variant="error" dot>{errors} Error</Badge>}{rows.length === 0 && <Badge>No connections</Badge>}</div></div>
        <div className="provider-card-foot"><span>{rows.length} connection{rows.length === 1 ? "" : "s"}</span><span>Manage <Icon>arrow_forward</Icon></span></div>
      </a>;
    })}</div>
...
function ProviderDetail({ provider }: { provider: Provider }) {
  ...
  return <section className="provider-detail">
    <a className="back-link" href="#/connections"><Icon>arrow_back</Icon>Back to Providers</a>
    <div className="provider-detail-head"><ProviderIcon provider={provider} alt={`${meta.label} logo`} size={54} /><div><h2>{meta.label}</h2><p>{connections.length} connection{connections.length === 1 ? "" : "s"}</p></div><a className="learn-link" href={meta.learnMore} target="_blank" rel="noreferrer"><Icon>open_in_new</Icon>Sign up / Learn more</a></div>
    ...
    <Card className="provider-detail-card">
      <div className="detail-card-head"><div><h3>Connections</h3><p>Priority, health, and availability for this provider.</p></div><div className="detail-actions">{connections.length > 0 && <Button variant="secondary" size="sm" icon="sync" loading={testingAll} onClick={testOneByOne}>Test Connection One-by-One</Button>}{testingAll && <Button variant="ghost" size="sm" icon="stop" onClick={() => { stopTests.current = true; }}>Stop</Button>}<label className="round-robin"><span>Round Robin</span><Toggle checked={roundRobin} label="Round robin" disabled={connections.length < 2 || testingAll} onChange={setRoundRobin} /></label></div></div>
      ...
      {!connectionsState.loading && connections.length === 0 && <div className="provider-empty-detail"><Icon>{provider === "codex" ? "lock" : "key"}</Icon><span>No connections yet</span><Button size="sm" onClick={() => provider === "codex" ? oauth() : setEditor("new")}>Add Connection</Button></div>}
      {connections.length > 0 && <><label className="select-all"><input type="checkbox" checked={selected.size === connections.length} onChange={() => setSelected(selected.size === connections.length ? new Set() : new Set(connections.map((connection) => connection.id)))} />Select All</label><div className="detail-connections">{connections.map((connection, index) => {
        const status = connectionStatus(connection, testResults[connection.id]);
        return <div className={`detail-connection-row ${connection.isActive ? "" : "is-inactive"}`} key={connection.id}>
          <input className="row-select" aria-label={`Select ${connection.name}`} type="checkbox" checked={selected.has(connection.id)} onChange={() => { const next = new Set(selected); next.has(connection.id) ? next.delete(connection.id) : next.add(connection.id); setSelected(next); }} />
          <div className="priority-arrows"><button disabled={index === 0 || roundRobin} aria-label={`Move ${connection.name} up`} onClick={() => reorder(index, -1)}><Icon>keyboard_arrow_up</Icon></button><button disabled={index === connections.length - 1 || roundRobin} aria-label={`Move ${connection.name} down`} onClick={() => reorder(index, 1)}><Icon>keyboard_arrow_down</Icon></button></div>
          <Icon>{provider === "codex" ? "lock" : "key"}</Icon>
          <div className="detail-connection-main"><strong>{connection.data.email || connection.name}</strong>{connection.data.email && connection.data.email !== connection.name && <small>{connection.name}</small>}<div><Badge variant={status.variant} dot>{status.label}</Badge><Badge>{provider === "codex" ? "OAuth" : "API Key"}</Badge><span>#{connection.priority}</span></div>{(testResults[connection.id]?.error || connection.lastError) && <small className="row-error">{testResults[connection.id]?.error || connection.lastError}</small>}</div>
          <div className="detail-row-actions"><button onClick={() => testConnection(connection)} disabled={busyId === connection.id}><Icon>{busyId === connection.id ? "progress_activity" : "network_check"}</Icon><span>Test</span></button><button className={connection.data.autoPing ? "is-active" : ""} title="After an observed rate-limit cooldown resets, send one minimal request. Disables itself if that request fails." onClick={() => act(connection.id, () => send(`/api/admin/connections/${connection.id}`, "PATCH", { data: { autoPing: !connection.data.autoPing } }), `Auto-ping ${connection.data.autoPing ? "disabled" : "enabled"}.`)}><Icon>bolt</Icon><span>Auto-ping</span></button><button onClick={() => setEditor(connection)}><Icon>edit</Icon><span>Edit</span></button><button className="danger-action" onClick={() => setDeleting(connection)}><Icon>delete</Icon><span>Delete</span></button><Toggle checked={connection.isActive} label={`${connection.isActive ? "Disable" : "Enable"} ${connection.name}`} disabled={busyId === connection.id} onChange={(active) => act(connection.id, () => send(`/api/admin/connections/${connection.id}/${active ? "activate" : "deactivate"}`, "POST"), `${connection.name} ${active ? "enabled" : "disabled"}.`)} /></div>
        </div>;
      })}</div><div className="connection-footer-actions"><Button variant="secondary" size="sm" icon="playlist_add" onClick={() => setBulkOpen(true)}>Bulk Add</Button><Button size="sm" icon="add" onClick={() => provider === "codex" ? oauth() : setEditor("new")}>Add</Button>{selected.size > 0 && <Button variant="danger" size="sm" icon="delete" onClick={async () => { if (!confirm(`Delete ${selected.size} selected connections?`)) return; try { await Promise.all([...selected].map((id) => send(`/api/admin/connections/${id}`, "DELETE"))); setSelected(new Set()); refresh(); } catch (reason) { notify(message(reason), "error"); } }}>Delete Selected ({selected.size})</Button>}</div></>}
    </Card>

    <Card className="provider-detail-card">
      <div className="detail-card-head"><div className="model-title-controls"><div><h3>Available Models</h3><p>Models exposed under this provider namespace.</p></div>{provider !== "anthropic" && <label className="thinking-select"><span className="visually-hidden">Thinking effort</span><select value={thinkingMode} onChange={(event) => setThinkingMode((event.target as HTMLSelectElement).value)}>{["auto", "minimal", "low", "medium", "high", "xhigh", "max"].map((level) => <option key={level} value={level}>Thinking: {level[0]!.toUpperCase() + level.slice(1)}</option>)}</select></label>}</div><div className="detail-actions">{models.some((model) => model.disabled) && <Button variant="secondary" size="sm" icon="restart_alt" disabled={modelBusy} onClick={() => setVisibility(models.filter((model) => model.disabled).map((model) => model.id), false)}>Enable All</Button>}{models.some((model) => !model.disabled) && <Button variant="secondary" size="sm" icon="block" disabled={modelBusy} onClick={() => setVisibility(models.filter((model) => !model.disabled).map((model) => model.id), true)}>Disable All</Button>}</div></div>
      <div className="model-add-row"><Input label="Model ID" value={newModel} placeholder={provider === "anthropic" ? "claude-3-opus-20240229" : provider === "codex" ? "gpt-5.6-sol" : "gpt-4o"} onInput={(event) => setNewModel((event.target as HTMLInputElement).value)} onKeyDown={(event) => { if (event.key === "Enter") addModel(); }} />{provider === "openai" && connections.length > 1 && <label className="field"><span className="field-label">Target connection</span><select value={targetConnection?.id ?? ""} onChange={(event) => setModelConnectionId(Number((event.target as HTMLSelectElement).value))}>{connections.map((connection) => <option key={connection.id} value={connection.id}>{connection.name} · {connection.data.prefix}</option>)}</select></label>}<Button size="sm" icon="add" disabled={!targetConnection || !newModel.trim()} loading={modelBusy} onClick={addModel}>Add</Button>{provider !== "codex" && <Button variant="secondary" size="sm" icon="download" disabled={!targetConnection} loading={modelBusy} onClick={importModels}>Import from /models</Button>}</div>
...
function ConnectionModal({ value, fixedProvider, onClose, onSaved, onError, onOAuth }: { value: MaskedConnection | "new" | null; fixedProvider?: Provider; onClose: () => void; onSaved: () => void; onError: (message: string) => void; onOAuth: () => Promise<void> }) {
  ...
  return <Modal key={value === "new" ? `new-${fixedProvider ?? "any"}` : value?.id ?? "closed"} isOpen={value !== null} onClose={onClose} title={editing ? `Edit ${value.name}` : "Add Provider"} size="md">
    {!editing && !fixedProvider && <div className="provider-picker">{(Object.keys(PROVIDERS) as Provider[]).map((id) => <button type="button" key={id} className={provider === id ? "is-selected" : ""} onClick={() => { setProvider(id); setForm(EMPTY_FORM); }}><ProviderIcon provider={id} alt={`${PROVIDERS[id].label} logo`} size={30} /><span><strong>{PROVIDERS[id].label}</strong><small>{PROVIDERS[id].description}</small></span></button>)}</div>}
    {isOAuthCreate ? <div className="oauth-panel"><Icon>lock</Icon><h3>Connect with ChatGPT</h3><p>Fast 9Router opens OpenAI sign-in, then stores the returned local OAuth session.</p><Button icon="login" onClick={onOAuth}>Continue with ChatGPT</Button></div> : <form className="modal-form" onSubmit={submit}>
...
function BulkConnectionModal({ provider, template, isOpen, onClose, onSaved, onError }: { provider: Provider; template?: MaskedConnection; isOpen: boolean; onClose: () => void; onSaved: () => void; onError: (message: string) => void }) {
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title={`Bulk Add ${PROVIDERS[provider].label}`} size="lg"><div className="modal-form"><p className="field-hint">{codex ? <>Paste a JSON array. Each account must include <code>accessToken</code>; refresh and identity tokens are optional.</> : <>Paste one API key per line. Endpoint, prefix, models, and priority copy from <strong>{template?.name}</strong>.</>}</p><textarea rows={10} value={text} onInput={(event) => setText((event.target as HTMLTextAreaElement).value)} placeholder={codex ? '[{"accessToken":"...","refreshToken":"...","email":"user@example.com"}]' : "sk-key-one\nsk-key-two"} /><div className="modal-actions"><Button variant="ghost" onClick={onClose}>Cancel</Button><Button loading={busy} disabled={!text.trim()} onClick={submit}>Import All</Button></div></div></Modal>;
}
...
function AliasModal({ isOpen, models, onClose, onSaved, onError }: { isOpen: boolean; models: string[]; onClose: () => void; onSaved: () => void; onError: (message: string) => void }) {
  ...
  const save = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); try { await send(`/api/admin/aliases/${encodeURIComponent(name.trim())}`, "PUT", { target: effectiveTarget }); setName(""); setTarget(""); onSaved(); } catch (reason) { onError(message(reason)); } finally { setBusy(false); } };
  return <Modal isOpen={isOpen} onClose={onClose} title="Add Model Alias" size="sm"><form className="modal-form" onSubmit={save}><Input required label="Alias name" value={name} placeholder="fast-cheap" onInput={(event) => setName((event.target as HTMLInputElement).value)} /><label className="field"><span className="field-label">Target model</span><select value={effectiveTarget} onChange={(event) => setTarget((event.target as HTMLSelectElement).value)}>{models.map((model) => <option key={model} value={model}>{model}</option>)}</select></label><div className="modal-actions"><Button type="button" variant="ghost" onClick={onClose}>Cancel</Button><Button type="submit" loading={busy}>Save Alias</Button></div></form></Modal>;
}
